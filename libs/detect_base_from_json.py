...
def decode_base2(data):
    """Decodes base 2 (binary) encoded data into bytes."""
    int_value = int_from_bin(data)
...
def decode_base10(data):
    ...
    return str(data).encode()  # Convert decimal string to bytes


def decode_base16(data):
    """Decodes base 16 (hexadecimal) encoded data into bytes."""
...
def get_length_in_base(data_length, base):
    ...
    bits_per_digit = math.log2(base)  # Each digit in the target base represents log2(base) bits

    return math.ceil(total_bits / bits_per_digit)
