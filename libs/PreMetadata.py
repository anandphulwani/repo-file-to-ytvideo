...
class PreMetadata:
    ...
    def parse(self, meta_str, premetadata_frame_count):
        ...
        if not (meta_str.startswith(main_delim) and meta_str.endswith(main_delim)):
            raise ValueError("PreMetadata.py: Invalid metadata format: missing start or end markers.")
        ...
        if tokens[0] != "PREMETADATA":
            raise ValueError("PreMetadata.py: Invalid metadata header: expected 'PREMETADATA'")
